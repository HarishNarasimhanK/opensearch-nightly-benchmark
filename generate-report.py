...
def run_aggregate_report(args):
    """Generate one unified report combining all 4 modes + trend analysis."""
    print("📊 Generating AGGREGATED nightly report (all 4 modes)...")
    
    date_str = datetime.now(timezone.utc).strftime('%Y-%m-%d')
    sections = []
    
    # 4 modes: clickbench, http_logs, clickbench-remote, http_logs-remote
    modes = [
        ("clickbench", ""),
        ("http_logs", ""),
        ("clickbench", "-remote"),
        ...
        for engine in ["parquet", "parquetLucene", "lucene"]:
            ...
            if os.path.isfile(csv_file):
                with open(csv_file, "r") as f:
                    csv_parts.append(f"=== {engine} ===\n{f.read()}")
        csv_content = "\n".join(csv_parts) if csv_parts else "(No CSVs found)"
        
        # Call Bedrock for this mode
        print(f"  [{mode_label}] Calling Bedrock...")
        try:
            response = call_bedrock_comparison(
                log_content, csv_content, workload, suffix, model_id=args.model
            )
            sections.append(f"<h2>📊 {workload.upper()}{' (Remote Store)' if suffix else ' (Single-Node)'}</h2>\n{response}")
        except Exception as e:
            sections.append(f"<h2>📊 {workload.upper()}{' (Remote Store)' if suffix else ' (Single-Node)'}</h2>\n<p>Failed: {e}</p>")
            print(f"  ERROR: {e}")
    
    # Trend analysis
    print("  [trend] Calling Bedrock...")
    trend_parts = []
    for workload, suffix in modes:
        csv_name = f"{workload}{suffix}"
        history = get_latest_results(args.bucket, csv_name)
        if history:
            trend_parts.append(f"Mode: {csv_name}")
    ...
    trend_str = "\n".join(trend_parts) if trend_parts else "No historical data available."
    
    try:
        trend_response = call_bedrock_trend(trend_str, model_id=args.model)
        sections.append(f"<h2>📈 7-Day Trend (All Modes)</h2>\n{trend_response}")
    except Exception as e:
        sections.append(f"<h2>📈 7-Day Trend</h2>\n<p>Failed: {e}</p>")
        print(f"  ERROR: {e}")
...
<head>
<meta charset="utf-8">
<title>Nightly Benchmark Report — {date_str}</title>
<style>
body {{ font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 1000px; margin: 40px auto; padding: 0 20px; line-height: 1.6; color: #333; }}
...
<p>{date_str} &nbsp;|&nbsp; All Modes (Single-Node + Remote Store) &nbsp;|&nbsp; ClickBench + HTTP Logs</p>
</div>
...
<p style="color:#999; font-size:0.85em;">Generated by opensearch-nightly-benchmark via Amazon Bedrock (Claude Opus 4.5)</p>
    ...
    if args.output:
        with open(args.output, "w") as f:
            f.write(full_report)
        print(f"\n💾 Aggregated report saved to {args.output}")
    ...
    try:
        ...
        print(f"☁️  Aggregated report uploaded to s3://{args.bucket}/{s3_key}")
    except Exception as e:
        print(f"WARNING: Failed to upload aggregated report: {e}")
...
def main():
    ...
    parser.add_argument("--aggregate", action="store_true",
                        help="Generate one aggregated report combining all 4 modes + trend")
